...
# Gemini integration — imported lazily so system works without it
sys.path.insert(0, os.path.dirname(os.path.dirname(__file__)))
try:
    from gemini_service import (
        gemini_counterfactual,
        gemini_explain_for_family,
        gemini_sbar_summary,
    )
    _GEMINI_SERVICE = True
except ImportError:
...
def get_ai_decision(vitals, hospitals=None, patient=None, ambulance_uid=None):
    ...
    # ── Step 7: Gemini enhancements ────────────────────────────
    gemini_data = {}
    if _GEMINI_SERVICE:
        try:
            # 7a. Counterfactual — paramedic action plan
            gemini_data['counterfactual'] = gemini_counterfactual(
                vitals, condition, guardrails_triggered,
                emergency_type, patient
            )
            # 7b. SBAR summary for receiving hospital
            best_hospital_name = ranked[0]['name'] if ranked else 'nearest hospital'
            eta_min = ranked[0].get('eta_minutes', 10) if ranked else 10
            gemini_data['sbar'] = gemini_sbar_summary(
                vitals, condition, guardrails_triggered,
                patient, best_hospital_name, eta_min, prep_items
            )
            # 7c. Family-friendly explanation
            gemini_data['family_explanation'] = gemini_explain_for_family(
                vitals, condition, emergency_type,
                best_hospital_name, eta_min
            )
        except Exception as _ge:
            gemini_data['error'] = str(_ge)

    # ── Step 8 (was 7): Audit log ──────────────────────────────
    write_audit_log(
    ...
    return {
        ...
        # Gemini fields (new — used by new UI panels)
        'gemini':                   gemini_data,
        'agent_steps': [
            ...
            f'Step 7: Gemini AI enhanced — {"counterfactual + SBAR + family note" if gemini_data and "error" not in gemini_data else "fallback (rule-based)"}',
            'Step 8: Decision written to audit log',
        ],
    }
